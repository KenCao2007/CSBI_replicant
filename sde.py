...
class BaseSDE(metaclass=abc.ABCMeta):
    ...
    def sample_traj_imputation_forward(
        ...
        if opt.problem_name == "mdp":
            obs_data, cond_mask = init_dist.sample(return_all_mask=True)
            gt_mask = torch.ones_like(obs_data)
            obs_mask = torch.ones_like(obs_data)
        else:
    ...
    # def sample_traj_imputation_forward_mdp(
    #         self,
    #         ts,
    #         policy,
    #         corrector=None,
    ...
    #     _assert_increasing('ts', ts)
    #     init_dist = self.p  # sample from data, not prior.
    #     obs_data, cond_mask = init_dist.sample(return_all_mask=True)  # opt.samp_bs
    #     obs_data, cond_mask = obs_data.to(opt.device), cond_mask.to(opt.device)
    ...
    #     res = [xs, zs, x_term, obs_data, cond_mask]
    #     return res
